...
<%@page import ="com.study.util.FileUtil0" %>
  ...
  <body>
    ...
    <c:forEach items="${fn:split(msg,'')}" varStatus = "st">
		${st.count }, ${st.current }<br>
 		...
 		<c:set var="member" value = '<%= new Member("hong", "길동2", "1234", "042-123-1234", "stage018@naver.com", 10) %>' />
 		id = ${member.mem_id }<br>
		...
		${FileUtil0.fancySize(15245558) }<br>
		...
		${fancy = (size) -> size < 1024 ? size += "bytes" : (size < 1045760 ? size / 1024.0 += "Kb" : size / 10485760.0 += "Mb"); '' }
